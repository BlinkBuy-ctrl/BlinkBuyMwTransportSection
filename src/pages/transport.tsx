...
import {
  ...
  Calendar, Plus, Bike, TrendingUp, Users, Shield, LayoutDashboard, ChevronRight, ChevronLeft
} from "lucide-react";
...
const POPULAR_ROUTES = [
  { from: "Lilongwe", to: "Blantyre", icon: "✈️" },
  { from: "Blantyre",  to: "Zomba",   icon: "🛣️" },
  { from: "Lilongwe", to: "Mzuzu",   icon: "🚐" },
  { from: "Lilongwe", to: "Airport", icon: "🛫" },
];
...
function normalizeToServiceCard(l: Listing) {
  return {
    ...l,
    priceDisplay: l.price_display, priceType: l.price_type,
...
// ─────────────────────────────────────────────────────────────────────────────
// Horizontal scroll strip — shows all uploaded listings with real images
// User scrolls DOWN the page normally, and when they reach this strip they
// can swipe LEFT / RIGHT through vehicle cards, then continue scrolling down.
// ─────────────────────────────────────────────────────────────────────────────
function ListingsStrip({ listings }: { listings: Listing[] }) {
  const scrollRef                       = useRef<HTMLDivElement>(null);
  const [canLeft, setCanLeft]           = useState(false);
  const [canRight, setCanRight]         = useState(false);

  // Sort: premium first, then by rating, then newest
  ...
  useEffect(() => {
    // Check arrows after data loads
    const el = scrollRef.current;
    if (el) setCanRight(el.scrollWidth > el.clientWidth + 4);
  }, [sorted.length]);
    ...
    <div className="mb-8">
      ...
      <div className="flex items-center justify-between mb-3">
        <h2 className="text-sm font-black text-foreground flex items-center gap-2">
          🚗 Browse Vehicles
          <span className="text-[10px] font-normal text-muted-foreground">({sorted.length} listed)</span>
        </h2>
        <span className="text-[10px] text-muted-foreground italic">swipe to explore →</span>
      </div>
      ...
      <div className="relative">
        {canLeft && (
          <button
            onClick={() => nudge("left")}
            className="absolute left-0 top-1/2 -translate-y-1/2 -translate-x-3 z-10 w-8 h-8 rounded-full bg-background border border-border shadow-md flex items-center justify-center hover:bg-muted transition-all active:scale-90"
          >
        ...
        {canRight && (
          <button
            onClick={() => nudge("right")}
            className="absolute right-0 top-1/2 -translate-y-1/2 translate-x-3 z-10 w-8 h-8 rounded-full bg-background border border-border shadow-md flex items-center justify-center hover:bg-muted transition-all active:scale-90"
          >
        ...
        >
          {sorted.map(l => {
            const gradient = VEHICLE_GRADIENT[l.vehicle_type] ?? "from-teal-700/60 to-teal-500/30";
            const priceText = l.price_display ?? (l.price ? `MK ${Number(l.price).toLocaleString()}` : "Negotiable");
            ...
            return (
              <Link
                ...
                className="flex flex-col rounded-2xl overflow-hidden border border-card-border bg-card hover:border-teal-400/70 hover:shadow-lg transition-all active:scale-95 shrink-0 cursor-pointer"
              >
                ...
                <div className={`relative h-24 bg-gradient-to-br ${gradient} flex items-center justify-center overflow-hidden`}>
                  ...
                  {/* Premium badge */}
                  {l.is_premium && (
                    <div className="absolute top-2 left-2 bg-yellow-400/90 text-[8px] font-black text-yellow-900 px-1.5 py-0.5 rounded-full">
                      ⭐ PRO
                    </div>
                  )}

                  {/* Dark gradient overlay at bottom of image for text legibility */}
                ...
                <div className="p-2.5 flex flex-col gap-1 flex-1">
                  {/* Title */}
                  <div className="text-[11px] font-black text-foreground leading-tight line-clamp-2">
                    {l.title}
                  </div>
                  ...
                  <div className="flex items-center gap-0.5">
                    ...
                    <span className="text-[9px] text-muted-foreground truncate">{l.location}</span>
                  </div>
                  ...
                  {l.rating > 0 && (
                    <div className="flex items-center gap-0.5">
                      ...
                      <span className="text-[9px] font-bold text-amber-500">{l.rating.toFixed(1)}</span>
                      {l.review_count > 0 && (
                        <span className="text-[8px] text-muted-foreground">({l.review_count})</span>
                      )}
                    </div>
                  )}
      ...
      {/* Divider before main grid continues */}
...
export default function TransportPage() {
  ...
  const [stripListings, setStripListings] = useState<Listing[]>([]);
  ...
  // Fetch strip listings — all active listings with thumbnail + key fields, cap 30
  useEffect(() => {
    supabase
      .from("listings")
      .select("id,title,vehicle_type,location,price,price_display,rating,review_count,is_online,is_premium,is_featured,from_city,to_city,thumbnail_url")
      .eq("status", "active")
      .order("is_premium", { ascending: false })
      .order("rating",     { ascending: false })
      .limit(30)
      .then(({ data }) => setStripListings((data ?? []) as Listing[]));
  }, []);
    ...
    <div className="w-full max-w-7xl mx-auto overflow-x-hidden">
      ...
      <div className="relative bg-[hsl(190,60%,10%)] px-4 pt-8 pb-6 overflow-hidden">
        ...
        <div className="max-w-2xl mx-auto relative z-10"
          ...
          <h1 className="text-2xl font-black text-white mb-1 leading-tight">
            Where are you<br/><span className="text-teal-400">going today?</span>
          </h1>
            ...
            {(["now","later"] as const).map(t => (
              <button key={t} onClick={() => setTripType(t)}
                className={`flex items-center gap-1.5 px-4 py-2 rounded-full text-xs font-bold transition-all duration-200 ${
                  tripType===t ? "bg-teal-500 text-white shadow-lg shadow-teal-500/30" : "bg-white/8 text-white/50 hover:bg-white/15 border border-white/10"
                }`}>
          ...
          <div className="flex gap-2">
            ...
            <button onClick={() => { setPage(1); setActiveTab("listings"); }}
      ...
      <div className="px-4 py-5 w-full max-w-7xl mx-auto overflow-x-hidden">
        ...
        {activeTab==="listings" && (
          <>
            {/* ════════════════════════════════════════════════════════════
                HORIZONTAL SCROLL STRIP — sits inside the vertical scroll.
                Shows all uploaded vehicle listings. Swipe left/right here,
                then continue scrolling DOWN past it as normal.
            ════════════════════════════════════════════════════════════ */}
            {stripListings.length > 0 && (
              <ListingsStrip listings={stripListings} />
            )}
                ...
                <div className="grid grid-cols-2 gap-2.5">
                  {POPULAR_ROUTES.map(r => (
                    <button key={r.from+r.to}
                      ...
                      className="flex items-center gap-3 bg-card border border-card-border rounded-2xl px-3.5 py-3 text-left hover:border-teal-500/50 hover:bg-teal-50/5 transition-all active:scale-[0.97] group">
            ...
            {/* Main grid — continues below the strip normally */}
            {loading ? (
              ...
              <>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  {listings.map((l,i) => (
                    <div key={l.id} style={{ animationDelay:`${i*60}ms`, animation:"fadeUp 300ms ease both" }}>
